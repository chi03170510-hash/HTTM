...
from config import UPLOAD_DIR, CAMERA_INDEX, MODEL_PATH
...
class CameraService:
    ...
    def start(
        ...
        with self._lock:
            ...
            source = camera_source if camera_source is not None else CAMERA_INDEX
            model_file = model_path if model_path is not None else MODEL_PATH

            # Check capture source
            cap = cv2.VideoCapture(source)
            if not cap.isOpened():
                raise RuntimeError(f"Không thể mở nguồn camera: {source}")
